<template>
  <div class="main">
    <div class="category d-flex justify-center">
      ...
      >
        <div class="category__item--content py-14">
          {{ category.cat }}
        </div>
      </div>
    ...
    <div class="card d-flex justify-center">
      <div class="card__body" v-for="item in getListJokes" :key="item.id">
        ...
        <div class="card__body--action" @click="goDetailJoke(item.id)">
          <span> SEE STATS</span>
          <img
            alt="down icon"
    ...
    <div class="view-more mb-24" v-if="listJokes.length > defaultTakeJoke">
      ...
      >
        ...
        <img
          class="pl-8"
          alt="down icon"
          srcset="@/assets/icon/down-icon.png"
        />
      </div>
    </div>
  </div>
...
const listCategory = ref<{ cat: string; color: string }[]>([]);
...
const fetchCategory = async () => {
  ...
  const maxColor = colorCategories.length;
  const [err, res] = await $api.jokeService.getAllCategory();
  if (!err) {
    listCategory.value =
      res?.map((cat, index) => ({
        cat,
        color: colorCategories[index % maxColor],
      })) || [];
  }
};
...
const goDetailJoke = (jokeId: string) => {
  router.push({ name: "detail", params: { id: jokeId } });
};
...
const getListCategory = computed(() => {
  return listCategory.value.filter(
    (_, index) => index < defaultTakeCategory.value
  );
});
